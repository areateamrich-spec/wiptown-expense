...
print(f"PORT={PORT} USE_PG={USE_PG}", flush=True)
...
# ── get fresh connection every call ──────────────────────────────────────────
def get_conn():
    if USE_PG:
        import psycopg2
...
# placeholder: ? for sqlite, %s for pg
def ph(n=1):
    p = '%s' if USE_PG else '?'
    return ','.join([p]*n)
...
def run(sql, params=(), fetch=False):
    """Run one statement, return rows if fetch=True"""
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute(P(sql), params)
    result = to_list(cur) if fetch else None
    conn.commit()
    cur.close()
    conn.close()
    return result or []
...
def upsert_exp(d):
    if USE_PG:
        run("""INSERT INTO expenses(id,name,amount,category,date,type,tmpl_id,slip)
            ...
            (d['id'],d['name'],d['amount'],d['category'],d['date'],
             d['type'],d.get('tmpl_id'),d.get('slip')))
    else:
...
def insert_mgen(month_key, tmpl_id):
    if USE_PG:
        run("INSERT INTO month_generated(month_key,tmpl_id) VALUES(%s,%s) ON CONFLICT DO NOTHING", (month_key,tmpl_id))
    else:
        run("INSERT OR IGNORE INTO month_generated(month_key,tmpl_id) VALUES(?,?)", (month_key,tmpl_id))
...
if __name__ == '__main__':
    ...
    print(f"🚀 running 0.0.0.0:{PORT} db={'pg' if USE_PG else 'sqlite'}", flush=True)
    srv.serve_forever()
